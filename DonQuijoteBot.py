...
DonQuijote_bot = BotHandler(token)
...
def main():
    ...
    while True:
        all_updates = DonQuijote_bot.get_updates(new_offset)
        ...
        if len(all_updates) > 0:
            for current_update in all_updates:
                ...
                # Else, it awsers the message
                    ...
                    DonQuijote_bot.send_message(
                        first_chat_id, first_chat_name + ', en El Quijote la palabra '+palabra+" aparece "+str(n)+' veces.')
                    new_offset = first_update_id + 1
